...
static void _mark_antinode_harmonic(map_t* map, coordinates_t a, coordinates_t b)
{
    const coordinates_t offset = {
        a.row - b.row,
        a.col - b.col,
    };

    coordinates_t node = b;
    ...
    while (true) {
        ...
        map->data[node.row][node.col] = ANTINODE;
    }
}
...
static void _mark_antinodes(map_t* map, char antenna, const coordinates_list_t antenna_list)
{
    for (unsigned i = 0; i < antenna_list.len; i++) {
        for (unsigned j = 0; j < antenna_list.len; j++) {
            ...
            _mark_antinode_harmonic(map, a, b);
            _mark_antinode_harmonic(map, b, a);
        }
    }
}
